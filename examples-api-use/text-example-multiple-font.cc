...
static int usage(const char *progname) {
  ...
  fprintf(stderr,
          "\t-f <font-file>    : Use given font line 1.\n"
          "\t-F <font-file>    : Use given font line 2.\n"
...
int main(int argc, char *argv[]) {
  ...
  const char *bdf_font_file2 = NULL;
  int x_orig = 0;
  int y_orig = 0;
  int brightness = 100;
  ...
  int opt;
    while ((opt = getopt(argc, argv, "x:y:f:F:C:B:O:b:S:")) != -1) {
    switch (opt) {
    case 'b': brightness = atoi(optarg); break;
    case 'x': x_orig = atoi(optarg); break;
    case 'y': y_orig = atoi(optarg); break;
    case 'f': bdf_font_file = strdup(optarg); break;
    case 'F': bdf_font_file2 = strdup(optarg); break;
  ...
  if (bdf_font_file2 == NULL) {
    fprintf(stderr, "Need to specify BDF font-file2 with -F\n");
    return usage(argv[0]);
  }
  ...
  if (!font1.LoadFont(bdf_font_file)) {
    fprintf(stderr, "Couldn't load font '%s'\n", bdf_font_file);
    return 1;
  }
  rgb_matrix::Font *font = &font1;
